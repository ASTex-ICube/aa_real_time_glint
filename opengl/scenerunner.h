...
public:
        ...
        samples(samples)
    {
        ...
        window = glfwCreateWindow(fbw, fbh, windowTitle.c_str(), NULL, NULL );
...
private:
    ...
    void mainLoop(GLFWwindow * window, std::unique_ptr<Scene> scene) {
        ...
        while( ! glfwWindowShouldClose(window) && !glfwGetKey(window, GLFW_KEY_ESCAPE)  && !sceneClosed) {
            GLUtils::checkForOpenGLError(__FILE__,__LINE__);
			
            sceneClosed = scene->update(float(glfwGetTime()), window);
            scene->render();
